...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "send_music":
        await q.message.reply_text("🎵 موزیک خود را ارسال کن")
...
def main():
    ...
    app.add_handler(
        CallbackQueryHandler(buttons, pattern="^(send_music|stats)$")
    )

    app.add_handler(
        CallbackQueryHandler(edit_buttons)
    )
